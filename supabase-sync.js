  ...
  window.nikiAdmin = {
    ...
    async statisticsNameRules() {
      const [aliases, ignored] = await Promise.all([
        client.from('statistics_name_aliases').select('alias_key,alias_name,canonical_name,canonical_key').order('alias_name'),
      ...
      const missing = errors.some(error => error.code === '42P01' || error.code === '42703' || error.code === 'PGRST205' || /statistics_(name_aliases|ignored_pairs)|canonical_key/i.test(String(error.message || '')));
    ...
    saveStatisticsAlias: (aliasKey, aliasName, canonicalName, canonicalKey) => client.from('statistics_name_aliases').upsert({ alias_key: String(aliasKey).slice(0, 160), alias_name: String(aliasName).trim().slice(0, 120), canonical_name: String(canonicalName).trim().slice(0, 120), canonical_key: String(canonicalKey || '').slice(0, 160) || null, updated_at: new Date().toISOString() }, { onConflict: 'alias_key' }),
